...
import { getContentSource, listContentBySource, createScheduledPost, updateGeneratedContent, archiveContent, bulkArchiveContent, getContentRevisions } from '../../infrastructure/database/supabase'
...
export default function ContentLibrary() {
  ...
  async function handleRevise() {
    if (!selectedContent || !account) return
    ...
    try {
      const result = await reviseContent(
        ...
        '', // No user guidance for now
        setReviseProgress
      )

      // Add the new revision to the content list
      setContent(prev => [result.revision, ...prev])

      // Switch to viewing the revised version
      setSelectedContent(result.revision)
      setReviseProgress('')
  ...
  async function handleLoadVersions() {
    ...
    try {
      const contentVersions = await getContentRevisions(selectedContent.id)
  ...
  function handleUseVersion(version) {
    // Switch to this version in the main view
                ...
                {isEditing ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      disabled={revising}
                      className="btn-secondary flex items-center text-purple-600 hover:text-purple-700 border-purple-200 hover:border-purple-300"
                    >
                      {revising ? (
                        <>
                          <svg className="animate-spin h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24">
                    ...
                    {(selectedContent.revision_of || selectedContent.revision_number > 0) && (
                      <button
                  ...
                  {viewingVersion ? (
                    <>
                      <div className="flex items-center justify-between mb-4">
                        ...
                        {viewingVersion.id !== selectedContent?.id && (
                          <button
